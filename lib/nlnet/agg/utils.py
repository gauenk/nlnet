...
class IndexedMatmul2Efficient(th.autograd.Function):
    @staticmethod
    def forward(ctx, x, y, I, chunk_size=64):
        ...
        for m_offset in range(0,m,chunk_size):
            ...
            y_full = th.cuda.FloatTensor(b,k,this_chunk_size,n).fill_(0)
            ...
            z_interm = th.cat([th.matmul(y_full[:,i_k:i_k+1,:,:], x_interm)
    ...
    def backward(ctx, grad):
        ...
        for m_offset in range(0,m,chunk_size):
            ...
            for i_k in range(k):
                grad_x += th.matmul(grad_chunk[:,i_k,:,:], y_full[:,i_k,:,:]).permute(0,2,1)

            del y_full
            grad_y_full = th.cat([th.matmul(x_interm, grad_chunk[:,i_k:i_k+1,:,:]) for i_k in range(k)], 1)
            del grad_chunk
            grad_y_chunk = grad_y_full.gather(2, If.permute(0,1,3,2)).permute(0,3,2,1)
            del grad_y_full
            grad_y_chunks.append(grad_y_chunk)

        grad_y = th.cat(grad_y_chunks, 1)
