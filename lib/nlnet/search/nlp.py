...
def init(cfg):
    return NLPSearch(cfg.k,cfg.ps,cfg.ws,cfg.wt,cfg.nheads,cfg.stride0,cfg.stride1,cfg.run_acc_flow)
...
class NLPSearch():
    ...
    def set_flows(self,vid,flows,aflows):
        self.search.set_flows(aflows,vid)

    def get_acc_flows(self,**kwargs):
        if self.run_acc:
            return self.run_acc_flows(kwargs['flows'])
        else:
            return kwargs['aflows']
